...
<PropsTable client:load props={[
  ...
  { name: 'sortBy', type: 'BTableSortBy[]', required: 'No', default: '[]', description: 'Sort configuration (v-model support). Also the table\'s INITIAL sort — when nothing is sorted, no sort params are sent at all (see Sorting below)' },
