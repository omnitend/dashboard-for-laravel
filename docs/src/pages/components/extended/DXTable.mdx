...
<PropsTable client:load props={[
  ...
  { name: 'card', type: 'boolean', required: 'No', default: 'true', description: 'Wrap the table in a card. The table renders flush to the card border (rows and stripes reach the edge) while the header and pagination stay padded; the card clips to its radius. Because it clips, slot content that positions itself outside the card and does not teleport is cut off at the edge — library components are fine (DDropdown teleports, filter menus are fixed), but a hand-rolled position:absolute overlay in a cell or header slot needs teleporting. Set false for a plain, borderless variant on the page background.' },
