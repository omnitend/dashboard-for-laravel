...
<DTable
  client:load
  items={[
    { mode: 'API Mode (Recommended)', prop: 'api-url="/api/products"', bestFor: 'Most applications. Clean separation, reusable endpoints, better caching.' },
...
<DTable
  client:load
  items={[
    { feature: 'Setup', apiMode: '1 prop: api-url', inertiaMode: '3 props: items, pagination, inertia-url' },
    { feature: 'Data Source', apiMode: 'Auto-fetched via axios', inertiaMode: 'Passed as page props' },
    { feature: 'Sorting/Pagination', apiMode: 'Automatic AJAX', inertiaMode: 'Full page reload via Inertia' },
    { feature: 'URL Changes', apiMode: 'No (AJAX only)', inertiaMode: 'Yes (browser URL updates)' },
    { feature: 'Loading State', apiMode: 'v-model:busy', inertiaMode: ':loading prop' },
    { feature: 'Backend', apiMode: 'JSON API endpoint', inertiaMode: 'Inertia render + PaginatedResource' },
  ]}
  fields={[
