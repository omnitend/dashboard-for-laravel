...
<PropsTable client:load props={[
  ...
  { name: 'card', type: 'boolean', required: 'No', default: 'true', description: 'Wrap the table in a card. The table renders flush to the card border (rows and stripes reach the edge) while the header and pagination stay padded; only the table region is rounded to follow the card corners, so the card itself does not clip and a position:absolute popover in the header slot can overhang it. Content in a cell slot is still bounded by the .table-responsive scroll container — teleport it (DDropdown and the filter menus already do) or pass responsive={false}. Set false for a plain, borderless variant on the page background.' },
  ...
  { name: 'deleteGuard', type: '(item) => string | null | Promise<string | null>', required: 'No', default: '-', description: 'Run when Delete is clicked, before the confirm and request. Return a message to block the delete and show it immediately (skips confirm + request); return null to proceed normally. Awaited, like saveGuard, so an async guard works; a guard that throws also blocks' },
  { name: 'saveGuard', type: '(item, data) => string | null | Promise<string | null>', required: 'No', default: '-', description: 'Run when the modal Save/Create is clicked, before the request (both the editUrl and createUrl paths). Return a message to abort the save and show it immediately; return null to proceed. AWAITED (as deleteGuard is), so it can wait for an async control that has not finished — an image upload still in flight would otherwise submit the previous media map, succeed, toast "saved" and close the modal with the image silently lost. item is null in create mode; data is the live form data. Aborting leaves the modal open with edits intact; a guard that throws also aborts' },
...
- **`select` vs `select-native`** — both are single-select dropdowns sharing the
  *same* options and value semantics; they differ only in the control.
  `'select'` is a typeahead (browse-on-focus, type-to-narrow); `'select-native'`
  renders a plain native `<select>` (`DFormSelect`) for consumers who prefer
  OS-native menu behaviour — a full-height menu and native keyboard/scroll. Pick
  `select-native` when native feel matters more than type-to-filter.
  `filterMultiple` is **ignored** for `select-native` (it is always single-select).
...
- `'select'` - Typeahead dropdown with options (automatically adds "All" option)
- `'select-native'` - Plain native `<select>` (same options/values as `'select'`, OS-native menu; single-select only)
...
- `'select'` - Exact match typeahead dropdown (`filterMultiple: true` matches ANY of the chosen values)
- `'select-native'` - Exact match via a native `<select>` (single-select; same derived options as `'select'`)
...
### Guarding a save (`saveGuard`)

`saveGuard` runs when Save/Create is clicked, **before** the request. Return a
message to abort — it is shown as a toast, no request is made, and the modal
stays open with the user's edits intact. Return `null` to proceed. It covers
both the `editUrl` and `createUrl` paths.

It is the Save-side twin of `deleteGuard`. Both are **awaited**, so either can
return a promise — and that is the point of this one. An async
control in an `edit-value` slot — an image upload — that has not finished when
Save is clicked will otherwise submit the *previous* media map; the request
succeeds, the toast says saved, the modal closes, and the image is gone with no
...
// Wait for an in-flight upload rather than saving without it.
const saveGuard = async (item, data) => {
  await uploader.value?.whenIdle();
  if (uploader.value?.failed) return 'The image failed to upload — try again.';
  return null;
};
</script>
...
<template>
  ...
  >
    <template #edit-value(image)="{ value, update }">
      <ImageUploader ref="uploader" :model-value="value" @update:model-value="update" />
    </template>
  </DXTable>
</template>
```

`item` is the row being edited, `null` in create mode (same as the
`edit-value` slot's binding); `data` is the live form data. Save is not
re-entrant while the guard is pending, so a second click cannot fire a second
request. A guard that *throws* also aborts, surfacing its message — a guard
that could not decide is not permission to save.

Because the guard is awaited, the modal can be dismissed *while it is pending*.
Any dismissal — Cancel, the header close button, Escape or a backdrop click —
abandons the save, so nothing is written for a record the user has closed. The
same holds for `deleteGuard`.
