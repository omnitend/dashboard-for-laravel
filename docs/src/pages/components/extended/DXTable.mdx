...
<PropsTable client:load props={[
  ...
  { name: 'saveGuard', type: '(item, data) => string | null | Promise<string | null>', required: 'No', default: '-', description: 'Run when the modal Save/Create is clicked, before the request (both the editUrl and createUrl paths). Return a message to abort the save and show it immediately; return null to proceed. Unlike deleteGuard it is AWAITED, so it can wait for an async control that has not finished — an image upload still in flight would otherwise submit the previous media map, succeed, toast "saved" and close the modal with the image silently lost. item is null in create mode; data is the live form data. Aborting leaves the modal open with edits intact; a guard that throws also aborts' },
...
It is the Save-side twin of `deleteGuard`, with one difference: it is
**awaited**, so it can return a promise. That is the point of it. An async
