...
import { h } from 'vue';
import DXTabbedForm from '../../resources/js/components/extended/DXTabbedForm.vue';
...
describe('DXTabbedForm', () => {
  ...
  describe('Conditional fields (when model predicate)', () => {
    it('reactively shows a field based on live form data', async () => {
      ...
      const screen = render(DXTabbedForm, { props: { form, fields, showSubmit: false } });
      await flush();
  ...
  describe('defineForm acceptance', () => {
    it('accepts a defineForm return without an explicit fields prop', async () => {
      ...
      const screen = render(DXTabbedForm, { props: { form: product, showSubmit: false } });
      await flush();
